import { API_ENDPOINT, httpCall } from '../utils/http.js';
import { shortDate } from '../utils/date.js';
import type { SearchResponse } from './response.js';
import type {
  FilterDate,
...
function createSearchUrl(params: FilterOptions) {
  ...
  const filters: [string, Record<string, unknown>, FilterOptions[keyof Omit<FilterOptions, 'dates' | 'prices'>]][] = [
    ...
    [FILTER_ENERGY_EFFICIENCY, FILTER_ENERGY_EFFICIENCY_ID, params.energyEfficiencies],
  ...
  if (params.dates) {
    const fromDate = 'from' in params.dates && params.dates.from ? shortDate(params.dates.from) : '';
    const toDate = 'to' in params.dates && params.dates.to ? shortDate(params.dates.to) : '';

    const value = 'now' in params.dates ? 'now' : (`${fromDate}|${toDate}` as FilterDate);
    url.searchParams.append(FILTER_DATE, value);
  }

  if (params.prices) {
    url.searchParams.append(FILTER_PRICE, `${params.prices.from ?? 0}|${params.prices.to ?? 10000000000}`);
  }

  return url;
}

export async function searchEstates(params: FilterOptions): Promise<EstatePreview[]> {
